...
class SiteEvaluationRequest(BaseModel):
    ...
    # Optional live signal overrides (for demo / testing without live adapter calls)
...
async def evaluate_site(req: SiteEvaluationRequest) -> SiteEvaluationResponse:
    ...
    observed_at = now  # signals are "current"

    scores = scorer.score_site_context(
        site_id=req.name,
        wind_speed_mps=req.wind_speed_mps,
        wind_observed_at=observed_at if req.wind_speed_mps is not None else None,
        ghi_wm2=req.ghi_wm2,
        solar_observed_at=observed_at if req.ghi_wm2 is not None else None,
